    ...
    try {
      ...
      const [todayData, statsData, recordsData, calendarData, holidaysData, openOdData] = !attendanceBlocked
        ? await Promise.all([
            ...
            queryClient.fetchQuery({ queryKey: ['attendance-stats', user?.id], queryFn: () => attendanceService.myStats(), staleTime: 0 }).catch(() => null),
            queryClient.fetchQuery({ queryKey: ['attendance-records', user?.id], queryFn: () => attendanceService.my({ limit: 14 }), staleTime: 0 }).catch(() => []),
